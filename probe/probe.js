...
function initProbe(options, inputValues){
	...
	*/
	...
	Probe.prototype.printRequest = function(req){
		...
		if(this.printedRequests.indexOf(k) == -1){
			req.url = this.getAbsoluteUrl(req.url);
			//var trigger = this.describeElement(this.curElement.element) + "." + this.curElement.event + "()";
			var json = '{"type":"request", "data":' + this.requestToJson(req) + "}";
			this.print(json);
	...
	Probe.prototype.waitAjax = function(callback){
		...
		this.pendingAjax = [];
		console.log("Waiting for ajaxs: "+xhrs.length+"=>"+xhrs);
  ...
  function dumpObj(obj, name, indent, depth) {
    ...
    //this.print("triggering events for " + this.describeElement(element));
      ...
      //this.print("isEventTriggerable: " + events[a] + ' : ' + (this.isEventTriggerable(event) ? 'yes':'no') + ' objectInArray: ' + (this.objectInArray(this.triggeredEvents, teObj)? 'yes':'no'));
    ...
    //this.print(element)
        ...
        var areas = element.getElementsByTagName("area");
		...
		for(a = 0; a < forms.length; a++){
			 req = this.getFormAsRequest(forms[a]);
			this.printRequest(req);
		}

        for(a=0;a < areas.length;a++){
            //this.print(areas[i].href);
             req = this.getAreaUrls(areas[a]);
            this.printRequest(req);
        }
	};

  Probe.prototype.getAreaUrls = function (area) {
    var url = area.href;
    return new this.Request("link", "GET", url);
  };
	...
	Probe.prototype.getFormAsRequest = function(form){
		...
		for(var a = 0; a < inputs.length; a++){
			...
			if(inputs[a].tagName == "INPUT" && inputs[a].type !== null){
                //this.print(inputs[a].tagName + " " + inputs[a].type);
