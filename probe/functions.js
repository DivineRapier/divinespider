...
function usage(){
	var usage = "Usage: hasajax [options] <url>\n" +
				...
				"  -I              load images\n" +
        "  -o <string>     set headers string with \\r\\n divided \n";
console.log(usage);
}
	...
	for(var a = 0; a < args.opts.length; a++){
      ...
      case "o":
        ...
        try {
          options.header = true;
          //console.log(options);
          //var header_file = fs.read(args.opts[a][1]);
          //console.log('\n\n' + header_file + '\n\n');
          options.setHeaders = parseHeader(args.opts[a][1]);
            for (var i = 0; i<options.setHeaders.length; i++) {
                console.log(options.setHeaders[i]);
            }
        } catch(e) {
          console.log(e);
          phantom.exit(1);
...
function parseHeader(str) {
  ...
  var lines = []; // str 切割成行,每行为 key: val(\r\n)
  var ret = [];   // 返回值, 把每一个 key-val 数组添加到里面
  var cnt = 0;
  ...
  for (var i = 0; i < lines.length; ++i) {
    ...
    if (pos > 0) {
      tmp[0] = lines[i].substring(0, pos).trim();
      tmp[1] = lines[i].substring(pos+1, lines[i].length).trim();
